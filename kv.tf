...
resource "azurerm_key_vault_access_policy" "example5" {
  key_vault_id = azurerm_key_vault.example.id
  tenant_id = data.azurerm_client_config.current.tenant_id
  #object_id = data.azurerm_databricks_workspace.workspac03.storage_account_identity.0.principal_id
  object_id = module.databricks_workspace_01.output.storage_account_identity[0].principal_id
  depends_on = [ azurerm_key_vault.example ]
  key_permissions = [
      "Get", "List", "Update", "Create", "Import", "Delete", "Recover", "Restore", "Decrypt", "Encrypt", "UnwrapKey", "WrapKey", "Verify", "Sign", "Purge", "Release", "Rotate", "GetRotationPolicy", "SetRotationPolicy",
...
#   key_vault_id = azurerm_key_vault.example.id
#   tenant_id = data.azurerm_client_config.current.tenant_id
#   object_id = data.azurerm_databricks_workspace.workspac03.managed_disk_identity.0.principal_id
#   depends_on = [ azurerm_key_vault.example ]
#   key_permissions = [
#       "Get", "List", "Update", "Create", "Import", "Delete", "Recover", "Restore", "Decrypt", "Encrypt", "UnwrapKey", "WrapKey", "Verify", "Sign", "Purge", "Release", "Rotate", "GetRotationPolicy", "SetRotationPolicy",
